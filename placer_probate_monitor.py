...
def parse_card(card) -> Notice | None:
    ...
    if time_el is not None:
        post_date = (time_el.get("datetime") or time_el.get_text(strip=True))[:10]
        if "Post Date" in (time_el.get_text() or ""):
            post_date = time_el.get_text(strip=True).replace("Post Date:", "").strip()
...
def paginate(start: date, end: date, max_pages: int = 10) -> tuple[list[Notice], list[str]]:
    ...
    for page in range(max_pages):
        ...
        # Stop if fewer than a full page of result cards.
        if len(soup.select("div.panel.panel-result")) < 24:
            break
...
def row_html(n: Notice) -> str:
    ...
    <tr>
      ...
      <td><strong>{html.escape(n.decedent or "—")}</strong><br>
          <a href="{html.escape(n.notice_url)}">{html.escape(n.case_number or "no case #")}</a></td>
...
def enrich_notices(notices: list[Notice], year: int) -> list[dict]:
    from ecourt_client import ECourtClient

    client = ECourtClient(pause=1.2)
    rows = []
    for notice in notices:
        portal = {"found": False}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dry-run", action="store_true", help="Print report; do not update state or email")
